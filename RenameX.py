...
class FileRenamerApp(QMainWindow):
    ...
    def generate_new_name(self, file_path, index, custom_rule=None):
        ...
        ext_pattern = self.ext_edit.text().strip()
        if self.replace_from.text():
            try:
                replace_froms = self.replace_from.text().split('/')
                replace_tos = self.replace_to.text().split('/')
                name = self.apply_multiple_replacements(name, replace_froms, replace_tos)
                if ext_pattern and original_ext:
                    original_ext = self.apply_multiple_replacements(original_ext, replace_froms, replace_tos)
            except Exception as e:
                print(f"替换处理异常: {str(e)}")
                QMessageBox.warning(self, "替换处理错误", f"处理文件 {base_name} 的替换规则时出错: {str(e)}")
        try:
            position = self.position_spin.value()
            if self.operation_type.currentText() == "插入":
                if position <= len(name):
                    name = name[:position] + self.edit_text.text() + name[position:]
            else:
                delete_count = self.delete_count.value()
                if position < len(name):
                    name = name[:position] + name[min(position + delete_count, len(name)):]
        except Exception as e:
            print(f"插入/删除处理异常: {str(e)}")
        try:
        ...
        try:
            ...
            if ext_pattern:
                ext_pattern = self.process_date_time_tags(ext_pattern)
        except Exception as e:
        ...
        try:
            new_name = pattern.replace('*', name).replace('#', seq_str).replace('$', random_str)
        except Exception as e:
            print(f"通配符替换异常: {str(e)}")
            new_name = name
        try:
            new_name = self.restore_direct_chars(new_name)
        except Exception as e:
            print(f"特殊字符还原异常: {str(e)}")
        if os.path.isdir(file_path):
            return new_name
        else:
            try:
                if ext_pattern:
                    processed_ext = ext_pattern
                    if "<*-" in processed_ext or ("<-" in processed_ext and "*>" in processed_ext):
                        processed_ext = self.process_truncate_pattern(processed_ext, original_ext)
                    if "\\" in processed_ext:
                        processed_ext = self.process_specific_char_pattern(processed_ext, original_ext)
                    if '?' in processed_ext:
                        processed_ext = self.process_question_marks(processed_ext, original_ext)
                    new_ext = processed_ext.replace('*', original_ext).replace('#', seq_str).replace('$', random_str)
                    new_ext = self.restore_direct_chars(new_ext)
                    if new_ext and not new_ext.startswith('.'):
                        new_ext = '.' + new_ext
                else:
                    new_ext = ext
            except Exception as e:
                print(f"扩展名处理异常: {str(e)}")
                new_ext = ext
